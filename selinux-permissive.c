...
unsigned int *find_selinux_enforcing(void) {
    char *sym_sel_read_enforce =
        (char *)kallsyms_lookup_name("sel_read_enforce");
    int i = 0;
    ...
    for (i = 0; i < 128; i++) {
        if (memcmp(&sym_sel_read_enforce[i], asm_sub, 4) == 0) {
            selinux_temp =
                *(unsigned int *)&sym_sel_read_enforce[i + 4] & 0x000FFFFF;
            selinux_temp = ((selinux_temp >> 4) & 0x0000F000) |
                           (selinux_temp & (0x00000FFF));
            selinux_temp |=
                (((*(unsigned int *)&(sym_sel_read_enforce[i + 8])) << 16) &
                 0xFFFF0000);
